  ...
  */
  public Adventure(File file) {
    ...
    try {
      ...
      for (int i = 0; i < itemsList.size() / TELEPORTATION_ITEMS_DIVISOR; i++) {
        teleportationItems.add(itemsList.get(rand.nextInt(itemsList.size())));
      }
    } catch (Exception e) {
  ...
  Room inspectItem(Room currentRoom, String itemInspected) {
    for(String item : teleportationItems) {
      if (item.toLowerCase().equals(itemInspected)
              && currentRoom.getItems().contains(itemInspected)) {
        Room newRoom = rooms.get(rand.nextInt(rooms.size()));
