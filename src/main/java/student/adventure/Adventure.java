...
public class Adventure {
  ...
  /** The instance of the Random class for this game */
  Random rand = new Random();
  /** The rooms for the given JSON */
  List<Room> rooms;
  /** The list of items that allow for teleporation. Is randomly generated by constructor */
  List<String> teleportationItems;
  /** The list of all items on the map */
  List<String> allItems;
  /** Keeps track of whether the user has found the ending room */
  ...
  /**
   * Constructor for the Adventure class. An invalid file will never be passed to this constructor
   * because of the try-catch logic in main, but the exception must still be checked.
   *
  ...
  public Adventure(File file) {
    ...
    try {
      ...
      // There may be some duplicate elements that occur in the list due to the nature of this
      // algorithm which was intentional because we do not want there to be guaranteed
      // (itemsList.size() / TELEPORTATION_ITEMS_DIVISOR) number of teleportation items.
      teleportationItems = new ArrayList<>();
      for (int i = 0; i < allItems.size() / TELEPORTATION_ITEMS_DIVISOR; i++) {
        teleportationItems.add(allItems.get(rand.nextInt(allItems.size())));
      }
    } catch (Exception e) {
  ...
  Room evaluateInput(Room currentRoom, String input) {
    ...
    try {
      ...
      if (input.toLowerCase().contains("go ") && direction == null) {
        ...
        return currentRoom;
      } else if (input.toLowerCase().contains("inspect ")) {
        currentRoom = inspectItem(currentRoom, standardizedInput);
  ...
  /**
   * Checks to see if the user is in the end room. Changes the class variable isEndOfGame to order a
   * game exit from readInput().
   *
   * @param currentRoom the room the user is currently in
  ...
  String standardizeInput(String input) {
    ...
    if (inputAsLowerTrimmed.indexOf("go") == 0
        || inputAsLowerTrimmed.indexOf("remove") == 0
        || inputAsLowerTrimmed.indexOf("add") == 0
        || inputAsLowerTrimmed.indexOf("inspect") == 0) {
      return inputAsLowerTrimmed.substring(inputAsLowerTrimmed.indexOf(" ") + 1);
   ...
   */
  String getDirectionsAsString(Room currentRoom) {
  ...
  String getRoomInfo(Room currentRoom) {
    return currentRoom.getDescription()
        ...
        + getDirectionsAsString(currentRoom)
        + "\n"
        + getItemsAsString(currentRoom);
  }

  /**
   * Checks to see if an item requested to be inspected by the user allows for teleportation and
   * if it does, teleports the user to a random room on the map.
   *
   * @param currentRoom the room the user is currently in
   * @param itemInspected the item the user wants to inspect
   * @return the currentRoom if the user has not inspected an item that allows for teleportation,
   *         otherwise a new random room different from the one originally passed.
   */
  Room inspectItem(Room currentRoom, String itemInspected) {
    for (String item : teleportationItems) {
      if (item.toLowerCase().equals(itemInspected)
              && containsCaseInsensitive(itemInspected, currentRoom.getItems())) {
        Room newRoom = currentRoom;
        // While loop is to make sure that the user doesn't teleport to the same room that they are
        // already in because that would be pretty boring.
        while (newRoom == currentRoom) {
          newRoom = rooms.get(rand.nextInt(rooms.size()));
        }
        System.out.println("You have teleported to " + newRoom.getName() + "!");
        System.out.println(getRoomInfo(newRoom));
        return newRoom;
      }
    }
    if (currentRoom.getItems().contains(itemInspected)) {
      System.out.println("There is nothing special about this item.");
    } else {
  ...
  /**
   * A trivial helper method that checks for string equality while ignoring case. Is used in
   * inspectItems.
   *
   * @param stringToBeChecked the string that either exists in the list or doesn't
   * @param stringList the list of strings to check the given string against
   * @return Whether or not the string is in the list
   */
  public boolean containsCaseInsensitive(String stringToBeChecked, List<String> stringList) {
    for (String string : stringList) {
      if (string.equalsIgnoreCase(stringToBeChecked)) {
        return true;
      }
    }
    return false;
